...
class DiffusionModel(keras.Model):
    ...
    def denoise(self, noisy_images, noise_rates, signal_rates, training):
        ...
        pred_noises = network(
            [noisy_images, tf.reshape(noise_rates, [-1]) ** 2], training=training
        )
        pred_images = (noisy_images - noise_rates * pred_noises) / signal_rates

        return pred_noises, pred_images

    def reverse_diffusion(self, initial_noise, diffusion_steps, start_step=0):
    ...
    def plot_images(
        ...
        return render_samples(num_cols, num_rows, generated_images, as_pillow=True, title=title)
